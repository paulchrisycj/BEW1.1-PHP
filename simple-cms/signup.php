...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
  ...
  if ($password != $confirmPassword) {
    echo "<h1>Password and confirm password must match!</h1>";
  } else {
    $insertQuery = "INSERT INTO users (name, email, password, role) VALUE (:name, :email, :password, :role)";
    $insertStatement = $db->prepare($insertQuery);
    $result = $insertStatement->execute(array(
      ":name" => $name,
      ":email" => $email,
      ":password" => $password,
      ":role" => "User"
